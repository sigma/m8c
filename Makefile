...
local_CFLAGS = $(CFLAGS) $(shell pkg-config --cflags sdl3 libserialport) -DUSE_LIBSERIALPORT -DAPP_VERSION=\"v$(VERSION)\" -Wall -Wextra -O2 -pipe -I. -DNDEBUG
...
libusb: local_CFLAGS = $(CFLAGS) $(shell pkg-config --cflags sdl3 libusb-1.0) -DUSE_LIBUSB=1 -DAPP_VERSION=\"v$(VERSION)\" -Wall -Wextra -O2 -pipe -I. -DNDEBUG
libusb: m8c

rtmidi: INCLUDES = $(shell pkg-config --libs sdl3 rtmidi)
rtmidi: local_CFLAGS = $(CFLAGS) $(shell pkg-config --cflags sdl3 rtmidi) -DUSE_RTMIDI -DAPP_VERSION=\"v$(VERSION)\" -Wall -Wextra -O2 -pipe -I. -DNDEBUG
rtmidi: m8c
