...
static RingBuffer *audio_rb = NULL;
...
static void SDLCALL audio_callback_sdl2(void *userdata, Uint8 *stream, int len) {
  ...
  uint32_t got = 0;
  if (!ring_buffer_empty(audio_rb)) {
    got = ring_buffer_pop(audio_rb, stream, (uint32_t)len);
  }
...
static void audio_capture_sdl2(void) {
  ...
  while (available > 0) {
    ...
    SDL_LockMutex(ring_mutex);
    ring_buffer_push(audio_rb, temp, got);
    SDL_UnlockMutex(ring_mutex);
...
int audio_initialize(const char *output_device_name, const unsigned int audio_buffer_size) {
  ...
  audio_rb = ring_buffer_create(AUDIO_RING_BUFFER_SIZE);
  if (!audio_rb) {
...
// No-op for SDL3 (audio is stream-based, no pumping needed)
